...
Each app card has a same-height screenshot well on top. Turns fills it with `public/apps/turns.png`. Vortex shows the stacked lockup (`vortex-mark-light.png` / `vortex-mark-dark.png`) until frames exist. Buckaroo and Stable show a placeholder. iOS AppIcons (`{id}-icon.png`, exported from each app’s Icon Composer document) sit beside the name below. Set `screenshot` / `icon` in `src/content/site.ts`. Set `storeUrl` to turn the “App Store — coming soon” line into a link (Turns links to the App Store already); apps with `Android` in `platforms` also get a Google Play line, which `playUrl` turns into a link. Platforms listed in `platformsSoon` (Turns: Mac and visionOS) render as dashed “coming soon” chips.
